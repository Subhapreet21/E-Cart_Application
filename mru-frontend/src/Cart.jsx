...
const Cart = () => {
  ...
  const generateInvoice = (paymentId) => {
    ...
    cart.forEach((item, index) => {
      doc.text(
        `${index + 1}. ${item.pname} - ₹${item.pcost} x ${item.quantity}`,
        20,
        y
      );
      y += 10;
  ...
  //     const url =
  //       "https://graph.facebook.com/v15.0/your_whatsapp_business_number/messages"; // Replace with your API endpoint
  ...
  return (
    <div className="cart-container">
      <button className="back-button cart-back" onClick={() => navigate(-1)}>
        ← Back
      </button>
      <h2>Your Cart</h2>
      {cart.length === 0 ? (
